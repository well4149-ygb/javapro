...
public class CT extends Thread {
	...
	public void run() 
	//thread에 있는 thread에서 동작하는 run 함수를 오버라이딩 
	{ super.run(); // 부모 thread에 run 함수 호출 
	try { BufferedReader tmpbuf = new BufferedReader(new InputStreamReader(System.in)); 
	//버퍼리더를 이용한 입력 객체 생성 
	PrintWriter sendWriter = new PrintWriter(m_Socket.getOutputStream()); 
	//PrintWriter의 sendWriter로 변수로 선언 후 객체를 생성 
	//객체를 생성할때 소켓의 출력 스트림을 인자로 받아서 생성 
	String sendString; //문자열 변수 sendString 새성 
	while(true) //무한 반복 
	{ sendString = tmpbuf.readLine(); //입력을 받아서 문자열 변수 sendString에 대입 
